...
class SGDmomentumOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in model.params.keys():
            velocitys[k] = momentum*velocitys[k] + learning_rate*model.grads[k]
...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in model.params:
            ...
            params[k] = params[k] - (learning_rate/np.sqrt(cache[k] + eps))*grads[k]
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in params.keys():
            ...
            m_k = momentums[k]/(1 - beta1**t)
            
            v_k = velocitys[k]/(1 - beta2**t)
            
            params[k] = params[k] - (learning_rate/(np.sqrt(velocitys[k]) + eps))*momentums[k]
